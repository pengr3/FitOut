...
- **Phase 2 (Host partition):** the exact production-build mechanism for a hard 404 on the apex /ops path needs curl-level verification, not assumption. **Deploy target is SETTLED, correcting STACK.md's open question 1: Vercel + Neon** — `vercel.json` was committed 2026-09-01 as `c6e43b0` (quick task `260901-0zf`), two days before this research, and the PM confirmed it 2026-09-04. What this phase must therefore research instead is **Vercel preview-deployment hostnames**: every preview gets a generated `*.vercel.app` host, so a Host-match or `allowedHosts` rule naming only the two production hosts breaks previews outright — or serves the ops surface from a preview host. A **named** `ops.` subdomain suffices; no wildcard, so no Vercel nameservers.
...
- ~~**Deploy target is unstated**~~ - **RESOLVED 2026-09-04. The target is Vercel + Neon.** The claim was false when written: `vercel.json` is tracked at HEAD, committed 2026-09-01 as `c6e43b0`. The residual risk moved rather than closed - it is now **Vercel preview hostnames** breaking the Host match (see Phase 2 above), and it is a requirement rather than an unknown.
