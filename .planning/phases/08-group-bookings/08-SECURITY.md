...
| T-08-79 | Tampering (concurrent) | two simultaneous confirmBooking | accept→**mitigate** | **closed** | Compare-and-swap checkout lease (`src/lib/payments/checkout-lease.ts`, `drizzle/0023`, claimed in `confirmBooking` before any PayMongo call); proven over independent connections in `tests/booking/checkout-lease-race.test.ts` (8 cases, M1 + M2a/2b/2c) and at the call site in `tests/booking/confirm-double-submit.test.ts` (6)-(10), mutations A-D. Closed 2026-08-01 by quick task `260801-kv2` |
...
| AR-08-02 ~~(active)~~ **RETIRED 2026-08-01** | T-08-79 | ~~The confirmBooking expire-before-create gate is an unlocked read-then-act, so a truly concurrent double-click can still mint two sessions. A `SELECT … FOR UPDATE` spanning two PayMongo round-trips is a worse anti-pattern; matches the accepted `updateDeclaredPax` (CR-02) precedent. Sequential double-submit (the observed failure) IS closed.~~ **OVERTURNED by quick task `260801-kv2`.** The acceptance rested on a false dichotomy: that the only way to close the race was a `SELECT … FOR UPDATE` spanning both PayMongo round-trips. A compare-and-swap lease (`UPDATE … WHERE <free> RETURNING`, one autocommit statement) holds **no lock across the network at all** — its row lock lives for the duration of that single statement and has committed before the first `fetch`. The reason the risk was accepted therefore no longer applies, and T-08-79 is now `mitigate` / `closed`. AR-08-01 (T-08-74) is UNAFFECTED and remains accepted. | operator | accepted 2026-07-29, retired 2026-08-01 |
