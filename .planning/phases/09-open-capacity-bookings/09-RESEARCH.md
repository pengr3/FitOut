...
|---|-------|---------|---------------|
| A1 | **OC-05 mechanism = advisory-lock counter (not units-as-spots).** This leans away from D-21's literal "reuse units-as-spots" steer, justified by the date-pass single-row model (OC-02) that D-21 predates. CONTEXT OC-05 explicitly hands the choice to research and D-112 sanctioned the counter. | Pattern 1 | If the user prefers a **structural** guarantee (constraint-enforced) over a procedural lock+test, switch to units-as-spots (accept the parallel seat table + status denormalization). Both ship the same race-test gate. **Recommend confirming with the user.** |
| A2 | **OC-04: reuse `listing.maxOccupancy` as the cap** (its original D-07 meaning "how many people the space holds"; the D-111 RSVP meaning never coexists on an open listing; already publish-required positive). | Pattern 1/5 | If a dedicated `open_capacity_cap` column is preferred for explicitness, add one; low cost either way. Flagged because CONTEXT OC-04 says "do not silently overload." |
| A3 | **OC-09: dedicated `per_head_price_cents` column** (open pricing is purely linear, no included base). | Pattern 5 | If reusing `included`/`extraHeadFee` is preferred, the quote fork changes; the dedicated column is cleaner and lower-risk. |
