...
human_verification:
  - test: "Run `npm run email:previews -- <address> --send` and open the inbox in Gmail web, Gmail Android and Apple Mail (one in dark mode); record each cell in 15-UAT-EMAIL.md; state the Outlook-desktop gap acceptance (or open it) in the same file's final table."
    ...
    warning: "READ W-2 FIRST. Review finding WR-05 is still open and unfixed in the tree (re-confirmed this pass: scripts/send-email-previews.ts:226 allow-lists only RESEND_API_KEY and EMAIL_FROM, so BETTER_AUTH_URL is unset and src/lib/email.ts:213 ships href=\"/\" ). Messages 6 and 7 of 23 will carry a dead CTA that is an artefact of the harness, not of the product. Fix the allow-list before the walk, or the 'is the CTA tappable' cell for those two sends grades the wrong thing."
...
file's own blind-spot list should name it. W-2 (WR-05) is an existing review finding that would
