...
### Punch-list
1. **[HIGH — FIXED `bc6a057`] Publish dead-end.** Photon autocomplete didn't fill `city`/`postal_code`; the publish gate required both and rejected silently (client checklist falsely said "ready"). Fixed 3 ways: (a) autocomplete falls back `city`→place name for locality picks; (b) `postalCode` made optional (OSM often lacks it; not needed for bookability); (c) the wizard's review checklist now checks city/region/country too, so it's honest instead of a silent reject. tsc + 23 tests green. **Follow-up (low):** add a regression test for the autocomplete→publish path (the original gap that let this ship); make address fields manually editable; `currency` defaults to `usd` → should be PHP.
