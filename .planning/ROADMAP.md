...
| 9. Open-Capacity Bookings | 5/16 | Executing (Waves 1-2 done — DDL live, `createOpenCapacityHold` under `pg_advisory_xact_lock`, mode-forked publish gate + OC-17 lock, required `openCapacity` on `WhenLabelInput`; Wave 3 started — 09-03 is the SC#3 gate, with BOTH lock-deletion mutations executed RED and restored) | - |
