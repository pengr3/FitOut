...
- [x] **HSURF-01** — On `/host/listings`, cards in a row **align**, and every action control stays
...
- [x] **HSURF-02** — Creating a listing **lands the host on the edit wizard**, not on a
...
**22 requirements across the two already-complete phases (18 · 18.1).**
...
**The 25 outstanding requirements, by phase** (written 2026-09-04; see `.planning/ROADMAP.md` for each
phase's goal and success criteria):

- **Phase 19 · 3** — `HSURF-01`, `HSURF-02`, `CI-01`. Independent of everything else; `HSURF-02` opens
  with a **reproduction gate**, not a code change, and `CI-01` is a **new fifth CI job**, never a
  widening of `gate-price-parity`.
- **Phase 20 · 6** — `OPS-07`…`OPS-12`. The Host-header partition (`src/middleware.ts` →
  `src/proxy.ts`) is the milestone's **one hard prerequisite**; `OPS-12` (the 404-cloak re-measure) sits
  **after** the new ops routes exist, and closes here because this is the phase after which the probe
