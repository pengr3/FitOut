...
| OPS-06 | Phase 18.1 · 18.1-13 (the reveal + its audit row) · 18.1-14 (the proof) · **a plan yet to be written (D-274)** | **Planned — and the reason it is still Planned CHANGED on 2026-09-03.** ⚠ **The hand-walk it was waiting for HAPPENED AND PASSED**: `18.1-EVIDENCE.md § P2` records **4 `/ops` readings** (320 and 1280, court and grove, before and after reveal, on a host row and a listing row) — the **Show contact** button reaches 44px, the row does not overflow before or after reveal, `Email` and `Phone` render as two `<dl>` rows, **focus landed on the email anchor**, a phoneless host rendered **"Not provided"**, and no support address appears anywhere. `OPS_QUEUE_ROW_HEIGHT` is also settled: re-measured by 18.1-13 against the row **with the Contact fact** (`h-132 lg:h-211` → `h-144 lg:h-223`, 528/844 → 576/892 against observed 576.13/892.09, rounded DOWN because the steps above sat 0.13px and 0.09px inside the 4px tolerance) and its derivation chain re-verified by 18.1-14 in `18.1-EVIDENCE.md § P4`. **So every clause this row has named since 18.1-01 is discharged.** ⚠⚠ **WHAT KEEPS IT OPEN IS A NEW PM DECISION TAKEN OFF THAT VERY READING — D-274 (`deferred-items.md § D9`)**: the revealed email and phone are **clickable and open a mail app**, and the PM has decided they must be plain, copy-pasteable text. **DECIDED, NOT IMPLEMENTED.** Removing the anchor **returns the queue row to strictly terminal** — the Phase 18 property 18.1-13 had to widen to ship the `mailto:` — so this is a tightening rather than a reversal, and it touches `ops-contact-reveal.tsx`, `tests/ops/ops-queue-row.test.tsx`'s `EXCLUDED_MAILTO` allowance, and `18.1-UI-SPEC.md § Surface 4`. ⚠ **The focus-lands-on-the-email-anchor guarantee must be REDESIGNED, NOT DELETED** — with no anchor there is nothing to focus, and a reveal must still announce itself to a screen reader; the live region is already there. Closing this row now would record as final a design the PM has already ruled against. 18.1-13 ships `revealHostContact` in `src/app/actions/ops-contact.ts` with `requireStaff()` as its FIRST statement, returning `{ email, phone }` and writing the audit row IN THE SAME CALL — so there is no shape in which a reveal happens and the record does not exist. The affordance is on BOTH queue rows (D-271: an ops question is usually about a listing, so the way to reach the person is on the row carrying the subject), and the values live in the client island's React state, never in a prop the RSC serialises. **FINDING F-6 is RESOLVED AND ARGUED**: 18.1-CONTEXT phrases D-271 as adding `email` and `phone` to both queue projections, which D-271's own warning forbids — so `src/lib/ops/review-queue.ts` is BYTE-UNCHANGED and the reveal returns a separate type. That makes the no-PII-in-the-payload claim structural rather than documentary: the queue item type has no field for a contact column, so no consumer can read one and `tsc` is what says so. **D-72 is measured on the SERIALISED whole row** with an unmistakable canary email and phone, and the ok-branch `meta` is asserted as the exact key set `["userId"]` — who looked at whom, never at what. Mutation-scored: deleting the audit row reddens three named cases, putting the email in `meta` reddens the D-72 case, keying the budget on the subject instead of the actor reddens the limiter case, and inserting a read or a limiter call ABOVE the guard reddens four and five. (jsdom computes no stylesheet and `document.activeElement` is not a rendered focus ring, which is why the browser half always needed a person — and why it is worth saying plainly that the person found something the gates could not.) |
...
- **Phase 18.1 · 5 · 4 complete, 1 planned** — written at plan time per D-239 by `18.1-01`. HVER-08 was moved to Complete by `18.1-11`, the last of its five named plans; LVER-05 by `18.1-12`, its only one; **HVER-06 and HVER-07 by `18.1-14`** on 2026-09-03, on the strength of the sandbox transcript and the hand-measure in `18.1-EVIDENCE.md § P1` and `§ P2`. **ONE row is left, and it is left for a reason that did not exist when this line was last written.** `OPS-06`'s outstanding proof — the revealed row at 320 and 1280, focus order included, which CI provably cannot see (D-24) — **was driven and it PASSED**. What holds the row open now is **PM decision D-274**, taken off that very reading: the revealed contacts are clickable and must become plain copy-pasteable text (`deferred-items.md § D9`, DECIDED and unimplemented). ⚠ **Phase 18's roadmap checkbox is therefore NOT ticked and phase 18.1 is NOT marked complete** — closing on evidence that describes a surface the PM has already decided against would make the record wrong about exactly the thing this phase exists to fix. The plan that implements D-274 moves this last row to Complete and takes the checkbox with it.
