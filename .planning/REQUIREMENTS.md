...
| HVER-06 | Phase 18.1 · 18.1-07 (the submit path) · 18.1-10 (the words) · 18.1-11 (the surface) · 18.1-14 (the proof) | Planned — **three of four contributors have landed and the checkbox stays open on purpose.** 18.1-07 ships the only non-test, non-backfill writer of a `host_verification` row and mints it at `pending` so the `/ops` queue can fill; 18.1-10 ships the words; 18.1-11 ships the surface under `/host` this requirement names — `/host/verify`, reached from a fourth `HostSignals` advisory row rather than from nav, with BOTH required fields at submission (the D-269 email gate as a checklist row over a server re-read, and the D-268 phone as a required `tel` input over a server re-parse). What is outstanding is 18.1-14's hand-walk, which this row has named as the proof since 18.1-01: nothing in `tests/design/**` or in CI can see a real host on a real phone completing a real sandbox session, and D-273's `is_desktop_allowed: false` makes that walk the only evidence for the clause that matters most. |
...
- **Phase 18.1 · 5 · 2 complete, 3 planned** — written at plan time per D-239 by `18.1-01`. HVER-08 was moved to Complete by `18.1-11`, the last of its five named plans; LVER-05 by `18.1-12`, its only one. **All three open rows now name `18.1-14`'s hand-walk as their only outstanding proof and nothing else**: HVER-06 and HVER-07 have from the start, and OPS-06 joined them when `18.1-13` shipped the reveal and its audit row (the code half is done; the browser half — the revealed row at 320 and 1280, focus order included — is not something CI can see, D-24). Phase 18's roadmap checkbox cannot be ticked until these land; the executor of each plan named above moves its row to Complete.
