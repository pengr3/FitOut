...
- [x] **DS-11**: Three named card patterns exist (ResultCard, RowCard, PanelCard) and every card surface in the app uses one of them rather than re-deciding padding, radius and hover locally, **with two named, measured exceptions** — `listing/listing-card.tsx` and `notifications/notification-item.tsx`, each proven structurally unable to adopt rather than skipped, and each enforced by name in `tests/design/card-pattern-coverage.test.ts` so a third surface joining them fails the build. *(Text amended 2026-08-31 by PM ruling at milestone close, following the D-26 precedent — which formally amended UI-SPEC AC#8 in four places rather than leaving the discrepancy implicit. `listing-card.tsx`: its `CardFooter` inside a whole-card `<Link>` triggers the HTML adoption-agency algorithm — measured with a real parser as `anchors parsed: 6, outer anchor child count: 0`. `notification-item.tsx`: its `href` is nullable by security design (`safeHref` refuses `javascript:`, `data:` and protocol-relative URLs) and it is a `divide-y` popover row, not a card. Ten of twelve surfaces adopt.)*
...
- [x] **BFLOW-08**: The post-payment view is a distinct confirmation moment — success mark, status, reference, exact amount, venue-local time with named timezone, address, where the copy was emailed, and what happens next — which decays into the normal booking-detail page on later visits — **COMPLETE 2026-08-31.** The moment, its full-screen geometry and its `history.replaceState` decay (confirmed branch only, D-89) were built and machine-proved by 7 green cases in `e2e/confirmation-decay.spec.ts` during Phase 13. The last mile, **WALK A**, was walked by the PM on 2026-08-31 against a REAL hosted PayMongo `sk_test_` checkout: booking `f6cef95d-8f5b-47c9-b9c8-5dff6c6cceef` / `FIT-6XNSQXG0`, paid on PayMongo's own page, confirmed by a real `checkout_session.payment.paid` webhook over the ngrok tunnel, landing on `GET /bookings/{id}?paid=1`. All four conditions confirmed by the PM: the moment renders; the URL rewrites to `/bookings/{id}` with no query string; a reload shows the ordinary detail page with no moment; Back leaves for the cross-origin PayMongo page.
...
- [x] **TRUST-02**: The booking reference is copyable, rendered in tabular figures, present on every status, and carried in the email subject line — **COMPLETE 2026-08-31.** The on-screen half was delivered and measured by Phase 13 (0px advance-width drift, `13-13-SUMMARY.md`). The **email subject line half** — Phase 15's by 13-CONTEXT D-78, which Phase 15 never received (`v1.1-MILESTONE-AUDIT.md` § *The two that are genuinely open*) — was closed by quick task `260831-rpt`: the subject is now `Your FitOut booking is confirmed — {space} ({reference})`, watched red before the change in `tests/notifications/confirmation-email.test.ts`.
- [x] **TRUST-03**: The cancellation policy is disclosed on the confirmation and in the confirmation email with concrete dates, not only at listing, checkout and cancel-review — **COMPLETE 2026-08-31.** The **on-screen** half was delivered by Phase 13 on both the detail page and the confirmation moment via `composePolicyDisclosure()`. The **confirmation email** half was closed by quick task `260831-rpt`: `sendBookingConfirmed` now carries the policy as a second paragraph in BOTH projections, with a concrete venue-local date. The sentence is composed once by `composePolicyEmailLine()` (`src/lib/booking/policy-disclosure.ts`) and derived from `LADDER` through `policySummaryLine` — so no surface, on screen or in the inbox, types a percentage or an hour, and the copy audit grep over `email.ts` + `policy-disclosure.ts` returns nothing outside comments.
...
- [x] **TRUST-05**: A booker can view and print an itemised receipt for a paid booking — **COMPLETE 2026-08-31.** The route, the itemisation, the print contract in both themes and price parity against Postgres on three shapes were delivered and machine-proved in Phase 13. The last mile, **WALK B**, was walked on 2026-08-31: booking `FIT-TBQWWBTR` rendered through Chrome's own print pipeline with **Background graphics OFF** (`printBackground: false`), once per theme. Measured in the print projection: no solid-black flood (court `lab(100 0 0)`, grove `lab(98.7 -2.0 -0.4)`); reference and Total both readable; status renders as the word “Completed” with **zero** pill elements; **zero** visible buttons or links; and the itemisation and Total cannot split across a page break because both PDFs are a single A4 page (`/Count 1`, MediaBox 595.92×842.88). PDFs reviewed by the PM.
...
| DS-11 | Phase 11 | Complete (text amended 2026-08-31 — two named exceptions) |
...
| BFLOW-08 | Phase 13 | Complete (WALK A, 2026-08-31) |
| TRUST-01 | Phase 13 | Partial |
| TRUST-02 | Phase 13 + quick `260831-rpt` | Complete |
| TRUST-03 | Phase 13 + quick `260831-rpt` | Complete |
| TRUST-04 | Phase 13 | Complete |
| TRUST-05 | Phase 13 | Complete (WALK B, 2026-08-31) |
...
- **Owner correction (2026-08-31):** the EMAIL halves of `TRUST-02` and `TRUST-03` were D-78's Phase 13 → Phase 15 handoff, and Phase 15 never received either ID (`v1.1-MILESTONE-AUDIT.md` § *The two that are genuinely open*). Quick task `260831-rpt` closed both. The scope mapping is unchanged — the requirements were always mapped and always counted; what changed is which unit of work executed the second half.
