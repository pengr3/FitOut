...
- **[11-17] An ancestor `loading.tsx` still shows its fallback on a child route, so `/invite/[token]` resolves THROUGH `/`'s search-grid skeleton.** The plan's interface note says the invite route gets its own `loading.tsx` *"so it does not inherit `/`'s"*. That overstates what a child boundary can do: a nested `loading.tsx` adds an INNER Suspense boundary, it does not suppress the ancestor's. **Measured**, byte offsets from `curl http://localhost:3100/invite/deadbeef` against `next dev` on 17 August 2026:

  | offset | marker | source |
  |---|---|---|
  | 23261 | `Loading spaces` / `skeleton-card-grid` | `src/app/(public)/loading.tsx`, in the shell |
  | 26966 | `hidden id=` | first streamed replacement… |
  | 27948 | `Loading this invite` / `skeleton-panel` | …which is `invite/[token]/loading.tsx` |
  | 29306 | `hidden id=` | the resolved invite card |

  So the visible sequence on a hard load is **outer fallback → inner fallback → content**, and the outer stage is a 6-cell card grid under "Find a space to play" on a route whose content is one 512px card. The swap needs no data (both fallbacks are static), so it is normally sub-frame — but it is visible on a slow connection, which is the only connection a loading state exists for.

  **This is inherent to nested boundaries, not specific to the invite route.** `(app)/bookings/loading.tsx` fronts `/bookings/[id]`, `/bookings/[id]/cancel` and `/bookings/[id]/group` the same way, and `(host)/host/loading.tsx` fronts all nine host routes. On those the outer shape is at least a plausible ancestor of the inner one; on `/invite/[token]` it is not, because `/` and an invite have nothing structurally in common beyond sharing the `(public)` layout.

  **Not fixed here, deliberately (deviation Rule 4 — architectural).** The structural fix is to scope `/`'s fallback with a nested route group: move `(public)/page.tsx` → `(public)/(home)/page.tsx` and `(public)/loading.tsx` → `(public)/(home)/loading.tsx`, leaving the `(public)` segment itself with no `loading.tsx`. URLs are unchanged (a parenthesised segment is erased), but it is a **page move**, and this phase has been bitten twice by route-tree surgery (11-19 flipped both remaining static routes; 11-15 measured `PublicHeader` costing 5 static routes → 3). **Whoever picks it up:** `tests/design/type-scale.test.ts`'s `DISPLAY_INVENTORY` keys on `src/app/(public)/page.tsx` AND on `src/app/(public)/loading.tsx`, and `tests/design/loading-coverage.test.ts` pins 27/20/7/20 by construction rather than by path — so the move is two inventory keys plus a mechanical before/after diff of the `○`/`ƒ` table from `npm run build`. Do not attempt it without that diff.

- **[11-17] The shared `CardGridSkeleton` is `gap-5`; the search grid it now fronts is `gap-4 lg:gap-6`.** `search-results.tsx`'s in-component pending state and `src/app/(public)/loading.tsx` both render the shared pattern (which is the point — `/` must not shimmer two different ways), but `ResultsGrid` is `grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 lg:gap-6` while the pattern is `grid gap-5 sm:grid-cols-2 lg:grid-cols-3`. That is **±4px per gutter, in opposite directions either side of `lg`** — the skeleton is 4px looser below the breakpoint and 4px tighter above it. The pattern matches its OTHER adopter (`/host/listings`, a `gap-5` grid) exactly.

  **Not fixed here:** the two real grids disagree with each other, so one pattern cannot match both without taking its geometry from the call site — and `invite-card.tsx`'s argument against that ("a pattern that takes a spacing prop has stopped deciding anything") is the house rule. **Whoever picks it up:** the choice is (a) unify the two real grids on one gutter, which is a design decision about two shipped surfaces, or (b) accept the 4px. Plan `11-21`'s ±2px `boundingBox()` comparison is the only thing that can see it; jsdom cannot (D-131), so `skeleton-measurements.test.ts` will stay green either way.

- **[11-18] RESOLVED — the `11-16` handoff above. `search-results.tsx`'s inline fetch error stays exactly as it is, and its `NON_EMPTY_STATE_DASHED` row is PERMANENT rather than pending.** 11-16 deferred the conversion because `ErrorStateProps.routeOut` is required and *"where a failed search sends you"* was a product decision it did not own. Plan 11-18 has now answered that question five times — `/` for root, `/bookings` for `(app)`, `/host` for `(host)`, `/login` for `(auth)`, `/` for `(legal)` — and answering it is what makes the sixth case decidable in the other direction.

  **The answer for a failed search is that there is no route out, and that is not a gap in the copy contract — it is the contract not applying.** `ErrorState`'s two-action rule exists because a BOUNDARY replaces the whole screen: retry covers a transient fault, and without a second action the user's only recourse is the back button into the same error. The inline block on `/` is not that. The header, the filters, the footer and the search form are all still mounted and working; the page IS the way out. And the only candidate destination is `/` — the page the user is already on — which would render a button that visibly does nothing. Rendering a dead action to satisfy a required prop is worse than the one-action block, and would make "two actions, always" mean less everywhere else it is enforced.

  **So the row is not a deferral any more.** Whoever reads `NON_EMPTY_STATE_DASHED` next: that row does not need converting when somebody finds time, and re-opening it should require an argument against the paragraph above, not merely a note that the count could be higher. The rule the two files jointly establish is worth stating once: **`ErrorState` is for a surface that has replaced a screen; an inline failure inside a working page is a different shape and legitimately has one action.**

- **[11-18] There is no `src/app/(public)/error.tsx`, so an error on `/` or `/invite/[token]` unmounts the public header and footer.** STATE-02's inventory is exactly five boundaries and `tests/design/error-boundaries.test.ts` pins that list by name, so a sixth was not invented here. The consequence is real and is recorded rather than discovered: `src/app/error.tsx` is declared at the ROOT, and a boundary replaces everything below the layout it sits beside — which includes `(public)/layout.tsx` and therefore `PublicHeader` and `SiteFooter`. A failed `/` renders a chrome-less error page, while a failed `/bookings` keeps its whole shell (because `(app)/error.tsx` sits inside `(app)/layout.tsx`).

  **Whoever picks it up:** the fix is a sixth `error.tsx` at `src/app/(public)/error.tsx` with the root's copy and the root's route out, and it is a two-file change — the boundary, plus a sixth row in `BOUNDARIES` in `tests/design/error-boundaries.test.ts` (which will go red in two places until the row exists, by design; probe (c) in that file's header is exactly this edit, watched). It is a decision rather than a chore, because a route out of `/` that points at `/` is the same dead action the item above rejects — the honest route out for the public group may be the header itself, in which case the answer is "keep the chrome", which is precisely what the sixth boundary buys.

- **[11-18] `src/app/global-error.tsx` renders a document with no `<title>`, which fails WCAG 2.4.2 (Level A).** The root layout's title template cannot reach that document any more than its stylesheet can, and a client component cannot export `metadata`. It was NOT guessed at: Next assembles the head for this file, React 19 hoists a `<title>` rendered anywhere in the tree, and choosing between "render `<title>` inside `<body>` and let React hoist it" and "render an explicit `<head>`" needs somebody to render one and look — which needs a forced root-layout failure, and nothing in this repository can currently produce one. Inventing head content in the one surface that must work when nothing else does is the wrong trade to make blind.

  **Addressed to `11-22`,** which is the plan that will have this surface in a real browser for its baselines: capture the tab title as part of that work and pick the spelling with evidence. Phase 17's axe pass will flag it if 11-22 does not.

- **[11-18] ⚠ FORWARD-WARNING — `next dev` serializes the full server error, message and absolute file path included, into the HTML. Production does not. Any future "no error text in the page" check must know which one it is measuring.** Measured on next@16.2.7, `/dev/throw` throwing `new Error("SENTINEL_LEAK_PROBE")`:

  | server | occurrences in `page.content()` | the flight error record |
  |---|---|---|
  | `next dev` | **1** | `{"digest":"1030820611","name":"Error","message":"SENTINEL_LEAK_PROBE","stack":[["DevThrowPage","C:\Users\…\.next\dev\server\chunks\ssr\….js",102,11,0,0,false]]}` |
  | `next build && next start` | **0** | `{"digest":"3530324580"}` — and nothing else |

  The plan's prescribed criterion (*"`page.content()` contains the string ZERO times"*) is therefore **false under the only server the e2e suite can drive** and **true where it ships**. `e2e/error-leak.spec.ts` asserts zero in the RENDERED document (scripts and the dev overlay removed) plus closure — every whole-document occurrence must lie inside one named `self.__next_f` channel — so a leak into any other carrier still fails. The production number is a RECORDED MEASUREMENT, not a re-runnable assertion: the throw affordance 404s in production, which is `T-11-THROWROUTE` working correctly.

  **Whoever upgrades Next, React or the App Router:** re-run the production half of that table by hand. Temporarily replace `/dev/throw`'s `NODE_ENV` guard with `export const dynamic = "force-dynamic"`, `npx next build && npx next start -p 3100`, request the route, count the string, then restore the guard and re-verify the 404. If React ever stops redacting, no test in this repository will notice.

- **[11-21] `tests/design/empty-state-adoption.test.ts` cannot see a prop delivered through a JSX SPREAD, so every `tone` / `title` / `titleAs` assertion in it is blind to a spread-authored call site.** Its `parse()` reads `node.attributes.properties` and skips anything that is not a `ts.isJsxAttribute` — a `JsxSpreadAttribute` therefore yields `null` for every prop. Measured while adding `/dev/theme`'s two empty states: authored as `<EmptyState {...FIXTURE} />` the `<EmptyState>` CALL SITE was still counted (the pinned total moved 13 → 15, correctly) but the `tone="positive"` panel was INVISIBLE to *"pins `tone="positive"` to exactly ONE empty state in the whole tree"*, which stayed green while a second positive panel had shipped.

  **Not relied on, in this plan.** The two call sites were rewritten with inline attributes precisely so the gate would see them, the assertion went red as it should, and it was then widened into a named two-entry set plus a separate product-only clause. **Whoever picks it up:** the hole is still open for any FUTURE surface — `<EmptyState {...props} />` inside a product route would pass the tree-wide positive-tone assertion and the heading-element assertion without either one looking at it. The fix is a guard-the-guard in that file: count call sites whose props could not be read at all, and assert that number is zero or that every such file is declared. It is a change to a shipped gate rather than to product code, which is why it is recorded here instead of taken opportunistically.

- **[11-21] Four of the five error boundaries have no dev throw affordance, so `e2e/overflow-320.spec.ts` measures 8 of its 12 named routes and skips 4 with named reasons.** Plan 11-18 shipped exactly one such route, `src/app/dev/throw`, and `src/app/dev/` sits under no route group — so it can only ever reach `src/app/error.tsx`. `(app)`, `(host)`, `(auth)` and `(legal)` are unreachable without four NEW `page.tsx` files inside four route groups, which moves `tests/design/loading-coverage.test.ts`'s pinned counts (currently 28 / 8) and adds four routes to the production route table.

  **What covers them instead, so the hole's real size is legible:** each of the four renders `patterns/error-state.tsx` inside its group's shell, and both halves are measured at 320px separately — the public shell by the reachable rows in that spec, the booker and host shells by `e2e/shell.spec.ts`'s AC#4 test at 320px, and the panel itself by `/dev/theme` section 12 in both themes. What is NOT covered is the composite. **Whoever picks it up:** the cheap version is one `src/app/dev/throw-in/[group]/page.tsx` per group behind the same build-time `NODE_ENV` guard, plus the `loading-coverage` bump — but read that gate's "a change here means a ROUTE WAS ADDED and somebody has to decide which side it is on" comment first, four times.

- **[11-21] CLOSURE ON THE `[11-17]` GUTTER ITEM ABOVE — `e2e/skeleton-geometry.spec.ts` does NOT see it, and the item stays open.** That entry ends *"Plan `11-21`'s ±2px `boundingBox()` comparison is the only thing that can see it"*. It is not, as built: the spec compares `CardGridSkeleton` against a resolved twin rendered on `/dev/theme` in the pattern's OWN grid classes (`grid gap-5 sm:grid-cols-2 lg:grid-cols-3`), because the two containers have to lay out at the same column width or the media comparison measures the wrapper instead of the constant. The ±4px disagreement is between the PATTERN and `ResultsGrid` on `/`, and nothing on `/dev/theme` renders `ResultsGrid`.

  **Whoever picks it up:** the measurement that would see it is a `boundingBox()` comparison taken on `/` itself, between `[data-testid="skeleton-card-grid"]`'s cells during the pending state and `[data-testid="result-card"]` after the results land — reachable with `e2e/helpers/served-document.ts`'s truncator, which already produces exactly those two states from one response. It was not added here because AC#17 is scoped to the DB-free page and `/`'s grid needs a non-empty catalogue.

- **[11-22] TWO DECLARED BASELINES ARE BLOCKED, both with argued reasons, and both are real open work with owners.**

  **`global-error` (1 baseline, and it also owes 11-18 an answer).** Nothing in this repository can render this surface: `/dev/throw` throws inside a *page* and `src/app/error.tsx` catches every page throw in the tree; `global-error.tsx` renders only when the ROOT LAYOUT throws. This closes out `[11-18]`'s `<title>` item above in the negative — that entry addressed the question to `11-22` on the assumption this plan would have the surface in a real browser, and it does not. **Whoever picks it up:** both cheap routes were refused deliberately and should stay refused unless someone argues otherwise. Reading a request signal in `src/app/layout.tsx` makes EVERY route dynamic and destroys the only two `○ Static` routes in the build — `11-19` measured exactly that counterfactual — and rendering the component inside a dev page nests a second `<html>` in the root layout's, so the baseline would be of a document that never ships. A build-time env flag (`NEXT_PHASE`-style, no operator-settable variable) on a throw in the root layout is the only shape that does not cost the static routes, and it edits the most load-bearing file in the app, so it is a scoped decision and not a side effect. **The coordinator decided on 2026-08-17 that it stays blocked**; this entry exists so that is a decision somebody can find rather than an absence.

  **`og-listing` (1 baseline).** Needs a published listing, which 11-UI-SPEC § GATE-01's own scoping rule forbids in this phase. Note the trap if you pick it up: the route answers **200** without a database — `og-facts.ts` returns null and the route serves its `GenericCard`, which `11-20` measured as byte-identical to the root card (25,844 B) — so a baseline shot without a fixture is green forever while covering the wrong card. **Whoever picks it up:** Phases 12-15 baseline product surfaces with their own fixtures; this row belongs to whichever of them seeds a listing.

- **[11-22] THE VR GATE CANNOT SEE A CHANGE CONFINED TO `--border` ON `--background`.** Measured while driving D-30's second OBSERVED RED, and it is why that proof failed 18 baselines rather than the 20 predicted. `toHaveScreenshot`'s default `threshold: 0.2` becomes a pixelmatch per-pixel cutoff of `35215 × 0.2² = 1408.6` on YIQ deltaSquared; the border/background pair measures **341.6** court (`rgb(229,229,229)` on `rgb(255,255,255)`) and **397.2** grove — four times under, so those pixels are not counted as different at all. Same pair `contrast-pairs.ts` carries in `EXCLUDED_PAIRS` at 1.26:1 / 1.28:1 for being a nearly invisible decorative divider: a gate cannot see what a person cannot see, and the two facts have one cause. **NOT a bug to fix by lowering the global threshold** — driving it toward 0 makes font antialiasing a failure on every run, and a gate that cries wolf is retried until green, which is the outcome this phase exists to remove. **Whoever picks it up:** if a future surface makes a border load-bearing, tighten `threshold` at THAT call site only. Recorded in `e2e/visual/surfaces.spec.ts`'s NOT COVERED footer as well, which is where a reader of the gate will actually meet it.
