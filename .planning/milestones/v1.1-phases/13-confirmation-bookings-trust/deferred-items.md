...
**13-15's `ProfileLink` 16x16 target is CLOSED by plan 17-06 (2026-08-29), under 17-CONTEXT D-196 —
and this row predicted the fix correctly, including the part that needed measuring.** The row above
ends *"the likely shape is padding on the link plus a re-measure of the cluster, not a bigger glyph"*.
That is exactly what shipped: `p-1.5` (6px) on the `<Link>`'s own `cn()` argument, taking the control
from **16×16 to 28×28**. `NAV_LINK_CLASS` is byte-unchanged (four header links share it), `size-4` on
the glyph is untouched, and both accessibility mechanisms — `aria-label="Profile"` on the wrapper,
`hidden sm:inline` on the label — stay exactly as shipped.

**The 226px budget this row was afraid of had 45.6px spare, and nobody had measured it.** The row
warns that *"the obvious `p-2` costs 16px of width in the exact cluster the label was dropped from"*.
Measured in real Chromium on `/profile` at 320×800, signed in:

| | court | grove |
|---|---|---|
| cluster before `p-1.5` | 178.4px | — |
| cluster after `p-1.5` | 190.4px | 191.6px |
| available | 224px (288px content box − 52px brand − one `gap-3`) | — |
| budget as the spec states it | 226px | — |

So the 200px / 226px pair the `ProfileLink` docblock carried was **an estimate, and both halves of it
were wrong** — it came from `11-UI-SPEC § Responsive behaviour`'s arithmetic (*"mode switch 96 + bell
32 + Profile 48 + 2 gaps 24 = 200px"*), and `measurements.ts`'s `AUTH_SLOT_ICON` docblock already
recorded one of its terms as false (the bell is `size-11`, 44px, not 32). Even `p-2` would have fit.
Nothing was shrunk, no label was dropped and no gap was reduced.

**And the finding is now under a gate rather than under a comment.** `collectControls`'s `site-header`
exclusion — written around this very control — is deleted, so `ProfileLink` is judged against the 24px
AA floor on every Phase-13 and Phase-14 case; the footer half is kept with its own measurement (all
five footer links are `display: inline` and exempt by WCAG 2.5.8's own exception). A new
`AC#22 / D-196` block in `e2e/overflow-320.spec.ts` asserts the 24px floor on both axes and the 226px
cluster budget, per theme, both watched red. `npx playwright test e2e/overflow-320.spec.ts
--project=chromium --workers=1` → **66 passed / 15 skipped**.
