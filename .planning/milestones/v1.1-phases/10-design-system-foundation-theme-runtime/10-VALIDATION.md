...
| DS-13 | No leak-pattern match under `src/app/**` or `src/components/**`; **and the build goes red on a deliberately injected leak** | static gate + **manual proof** | `npm run test:design -- leak` then `npm run build` | 10-01 · 10-02 · 10-14 (palette-class remediation, D-15) · 10-17 | ❌ W0 |
