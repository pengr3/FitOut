...
// two controls that are about to appear (`AUTH_SLOT_ICON` = the navigation trigger's 32px,
// `NOTIFICATION_BELL_BOX` = the bell's 44px) say what is happening without claiming to know which of
// the anonymous or signed-in clusters will win.
//
// ═════════════════════════════════════════════════════════════════════════════════════════════════
...
// whole-cluster fallback there would duplicate the compact cluster inside an already-populated one.
// `BellSlotSkeleton` is the bell's own box and nothing else, so the pending and resolved widths are
// equal.
...
export function AuthSlotSkeleton() {
  return (
    ...
    >
      <Skeleton className={cn(AUTH_SLOT_ICON, "rounded-lg")} />
      <Skeleton className={cn(NOTIFICATION_BELL_BOX, "rounded-lg")} />
    </div>
  );
}
