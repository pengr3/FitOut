    ...
    <div className="space-y-3 rounded-card border border-border bg-card p-4 shadow-card">
      ...
      {showGroupInput ? (
        <div className="flex flex-wrap items-end gap-2">
          <label className="grid gap-1 text-sm font-medium" htmlFor="group-party-size">Number of people
          ...
          <Button type="button" variant="brand" size="touch" disabled={partySize === null} onClick={() => partySize !== null && onSubmitGroup(partySize)}>See spaces</Button>
        </div>
      ) : null}
    </div>
  );
