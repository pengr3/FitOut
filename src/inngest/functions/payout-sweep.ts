...
//      No match ⇒ leave the money Held + operator alert + fire NOTHING (never wallets[0], never cross-pay).
//
...
export type PayOneResult =
  ...
  | { status: "skipped-no-wallet" } // no activated wallet correlates to this host → money stays Held
 ...
 *   3. CORRELATE the wallet to THIS booking's host (`wallet.id === b.paymongoAccountId`). No match ⇒ leave
 *      the row Held, alert an operator, fire NOTHING — NEVER fall through to wallets[0] or a wrong host.
 *   4. FIRE the inhouse transfer of exactly `net_cents` (D-52) to the correlated wallet.
 *   5. RELEASE: move the ledger Held → Processing with the transfer id. On a transfer throw, mark it
 *      `failed` and alert (retryable operator review).
 *
...
export async function payOne(dbConn: DbConn, b: DuePayout): Promise<PayOneResult> {
  ...
  if (!wallet) {
    // No wallet belongs to this host: the money STAYS Held (correct fail-closed). Alert; fire nothing.
  ...
  //     inside createBatchTransfer). (5) On success, RELEASE Held → Processing; on throw, mark failed.
  try {
    ...
    await dbConn.execute(sql`
      UPDATE host_payout_ledger SET state = 'failed', updated_at = now() WHERE booking_id = ${b.bookingId}
    `);
    console.error("[payout-alert] transfer create failed", { bookingId: b.bookingId });
