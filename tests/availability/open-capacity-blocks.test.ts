...
beforeAll(async () => {
  ...
  vi.resetModules();
  ({ addBlock } = await import("@/app/actions/blocks"));
  ...
  await testDb.db.insert(listing).values({
    ...
    // ⚠️ DELIBERATELY ADVERSARIAL, DO NOT REMOVE (the 09-07 lesson). A drop-in listing may legitimately still
    // carry hourly/day rates — 09-06 requires a per-head price but never CLEARS the exclusive columns — so a
    // fork that keyed off a null rate rather than off the persisted occupancy_mode cannot pass here.
    hourlyRateCents: 90_000,
    dayRateCents: 400_000,
    timezone: TIMEZONE,
  });

  // Open EVERY weekday, so each case owns its own date without a "closed that day" surprise.
...
describe("CR-02 — a host block must withdraw a drop-in date from sale", () => {
  ...
  it("3 · the CLAIM refuses a blocked date and mints NO booking row", async () => {
    ...
    // …and only then, that the caller was actually refused.
