// QK-IR9 / AUTH-02 / AUTH-03 — LAYER 1 (middleware as a pure function) + THE LOOP CLOSURE.
//
...
// Reset: true` deletes the session ROW, nothing clears the httpOnly cookie, and middleware's
...
// middleware DELEGATES to it, and /login is only reachable if middleware then lets the post-clear
...
//   middleware -> /auth/session-check -> apply the Set-Cookie to a simulated cookie jar -> middleware
// and asserts the second middleware call does NOT redirect.
//
...
/** The `location` of a middleware redirect, or null when it passed the request through. */
...
describe("LAYER 1 — middleware delegates instead of terminating the journey", () => {
  it("case 1: a cookie-bearing /login request is sent to the verifier, NOT to /", () => {
    const location = redirectOf(middleware(req("/login", STALE)));
  ...
  it("case 2: the post-reset ?reset=1 notice survives the delegation hop", () => {
    const location = redirectOf(middleware(req("/login?reset=1", STALE)));
  ...
  it("case 3: with NO cookie, /login passes straight through (the state after the heal)", () => {
    expect(redirectOf(middleware(req("/login")))).toBeNull();
    expect(redirectOf(middleware(req("/login", "")))).toBeNull();
  });
  ...
  it("case 4 (LOOP GUARD): a request already carrying the marker is passed through", () => {
    ...
    expect(redirectOf(middleware(req(`/login?${CHECKED_PARAM}=1`, STALE)))).toBeNull();
    expect(redirectOf(middleware(req(`/signup?${CHECKED_PARAM}=1`, STALE)))).toBeNull();
    expect(redirectOf(middleware(req(`/login?reset=1&${CHECKED_PARAM}=1`, STALE)))).toBeNull();
  });

  it("case 5: /signup delegates identically", () => {
    const location = redirectOf(middleware(req("/signup", STALE)));
    expect(location!.pathname).toBe(SESSION_CHECK_PATH);
...
describe("LAYER 2 — the loop closure: a real reset leaves /login reachable", () => {
  it("case 6: middleware -> session-check -> browser jar -> middleware, in one process", async () => {
    ...
    const hop1 = redirectOf(middleware(req("/login?reset=1", jarHeader(jar))));
    ...
    expect(redirectOf(middleware(req(`${hop2.pathname}${hop2.search}`, jarHeader(jar))))).toBeNull();
    ...
    expect(redirectOf(middleware(req("/login", jarHeader(jar))))).toBeNull();
  });
  ...
  it("case 6b (constraint 4): the same journey with a VALID session still ends at /", async () => {
    ...
    const hop1 = redirectOf(middleware(req("/login", jarHeader(jar))))!;
    expect(hop1.pathname).toBe(SESSION_CHECK_PATH);
