...
async function seedListing(id: string, hostId: string, title: string) {
  await sql`
    INSERT INTO "listing" (
      ...
      ${50000}, ${300000}, ${"php"}, ${"request"}::booking_mode, ${"published"}::listing_status, ${"approved"}::listing_review_state,
      now(), now(), now()
    )
  `;
...
test.beforeAll(async () => {
  ...
  await sql`
    ...
    VALUES (${hostBookableId}, ${"approved"}::host_verification_status, ${"manual"}, now(), now())
  `;
